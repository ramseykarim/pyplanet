...
def alpha(freq,T,P,cloud,cloud_dict,otherPar,units='dBperkm',path='./',verbose=False):
    ...
    for f in freq:
        ...
        if otherPar.has_key('ice') and otherPar['ice'] > 0.0:
            fraction = cloud[cloud_dict['H2O']]/0.9   # g/cm^3 / g/cm^3
            e = water(f,T)
            alpha+= acloud(k,fraction,e)
        if otherPar.has_key('water') and otherPar['water'] > 0.0:
            fraction = cloud[cloud_dict['SOLN']]/1.0
            e = water(f,T)
            alpha+= acloud(k,fraction,e)
        if otherPar.has_key('nh4sh') and otherPar['nh4sh'] > 0.0:
            ...
            e = n**2
            alpha+= acloud(k,fraction,e)
        if otherPar.has_key('nh3ice') and otherPar['nh3ice'] > 0.0:
            ...
            e = n**2
            alpha+= acloud(k,fraction,e)
        if otherPar.has_key('h2sice') and otherPar['h2sice'] > 0.0:
